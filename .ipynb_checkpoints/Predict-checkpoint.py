...
def __init__(sOutputSymbol, sModelType, ixToSimulate):
    ...
    ## Scale Data
    dfScaledOhlc = pd.DataFrame(index = ixToSimulate, columns  = dfOhlc.columns)
    for sColumn in dfOhlc.columns:
        ...
        dfToScale = pd.DataFrame(dfOhlc.loc[ixToSimulate, sColumn])
    
        dfScaledOhlc.loc[ixToSimulate, sColumn] = np.reshape(oScaler.transform(dfToScale), (-1))
    ...
    ixToSimulate= ixToSimulate.drop(ixNas, errors = 'ignore') 
    ...
    for tplColumn in list(dfOutput.columns):
        dfOutput.loc[:, tplColumn] =  dfOhlc.loc[ixToSimulate][(tplColumn[1])].shift(-tplColumn[0])
    ...
    ixToSimulate = ixToSimulate.join(axMerged, how = "inner")
    
    
    dfInputToSimulate = dfInput.loc[ixToSimulate]
    aInputToSimulate = np.reshape(dfInputToSimulate.values, (dfInputToSimulate.shape[0], iBackwardTimeWindow, iNrInputFeatures))
    
    dfOutputToSimulate = dfOutput.loc[ixToSimulate]
    ...
    dfPrediction = pd.DataFrame(data = aPrediction, index = ixToSimulate, columns = aIxOutputColumns)
    
    aActual = aOutputToSimulate.reshape((-1, iForwardTimeWindow * iNrOutputFeatures))
    dfActual =  pd.DataFrame(data = aActual, index = ixToSimulate, columns = aIxOutputColumns).copy()
