...
def main():
    ...
    for i_index, i_row in df_models.iterrows():
        model_id = i_row["ID"]
        df_input, df_time_steps_input = Train_Models.get_feature_values(model_id, "1",True)
        df_output , df_time_steps_target = Train_Models.get_feature_values(model_id, "2", False)
        
        feature_size_x, window_length_x = Train_Models.get_dimension_size(df_time_steps_input)
        feature_size_y , window_length_y = Train_Models.get_dimension_size(df_time_steps_target)
